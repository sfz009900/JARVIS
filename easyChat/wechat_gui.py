            ...
            try:
                ...
                try:
                    ...
                    if content.startswith('file:'):
                        file_path = content[5:]
                        self.wechat.send_file(target, file_path, True)
                    elif content.startswith('at:'):
                        at_content = content[3:]
                        self.wechat.at(target, at_content, True)
                    else:
                        self.wechat.send_msg(target, content, True)
...
def main(message_queue=None):
    app = QApplication(sys.argv)
    ex = WechatGUI(message_queue)
    sys.exit(app.exec_())
